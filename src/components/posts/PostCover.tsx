...
function DateTag({ date }: { date: Date }) {
    ...
    if (date.getTime() <= pivotDate.getTime()) {
        return (
            <p className="select-none text-gray-600 text-xs font-bold"></p>
        )
    }

    return (
        <p className="select-none text-gray-600 text-xs font-bold">{date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
    )
}
