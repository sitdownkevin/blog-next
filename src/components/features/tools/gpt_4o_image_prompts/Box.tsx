...
export function Box({
  ...
  const imageVariants = {
    top: {
      ...
      x: 0,
      y: 0,
      transition: { duration: 0.4, ease: [0.4, 0, 0.2, 1] as const }, // Smoother cubic-bezier
    },
    bottom: {
  ...
  return (
    ...
    >
      <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 bg-card border border-border rounded-2xl break-inside-avoid">
        {" "}
        {/* Add break-inside-avoid */}
        <CardHeader className="p-4 flex flex-row items-center justify-between">
          {" "}
          {/* Use flex to position title and button */}
          <div className="cursor-pointer" onClick={toggleExpanded}>
            {" "}
            {/* Wrap title/desc for click */}
        ...
        <CardContent className="p-4 pt-0 flex flex-col items-center gap-4">
          ...
          >
            {isExpanded ? prompt : `${prompt.substring(0, 50)}...`}{" "}
            {/* Show truncated prompt when collapsed */}
          </motion.div>
        </CardContent>
        {/* Tags in Footer */}
        {tags && tags.length > 0 && (
